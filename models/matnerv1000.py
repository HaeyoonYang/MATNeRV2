...
class NeRVEncoding(nn.Module):
    ...
    def __init__(self, size, channels, **kwargs):
        ...
        T, H, W = self.size
        C = self.channels
        self.Cg = int(self.channels * kwargs['glob_chans'])
        ...
        self.global_feat = FeatureGrid((1, H_grid, W_grid, self.Cg), init_scale=kwargs['grid_init_scale'] * 3/2)        # global feature
    ...
    def forward(self, idx: torch.IntTensor, idx_max: tuple[int, int, int], padding: tuple[int, int, int]):
        ...
        # Add global and temporal grid features
        enc = self.global_feat() + enc.repeat(1, 1, 1, 1, self.Cg // self.channels)
...
class NeRVDecoder(nn.Module):
    ...
    def __init__(self, logger, input_size, input_channels, output_size, output_channels,
        ...
        for i, scale in enumerate(self.scales):
            ...
            if i >= self.backbone_depths:
                if i == self.backbone_depths:
                    ...
                    C_C1 = C_C2
                    C_C2 = max(divide_to_multiple(C_C2, self.channels_reduce, self.channels_reduce_base), self.channels_min)
                ...
                # Encoder
                _c_enc_cfg = get_encoding_cfg('upsample', 0, size=(T2, H2, W2, C_C2), scale=scale, **c_enc_cfg)
                if _c_enc_cfg['type'] != 'none+none':
                    self.c_blocks[-1].append(PositionalEncoder(scale=scale, channels=C_C2, cfg=_c_enc_cfg))

                # Conv blocks
                _c_block_cfg = cfg_override(c_block_cfg, type=c_block_cfg['type'], C1=C_C2, C2=C_C2, kernel_size=self.kernels[i], depths=self.depths[i])
        ...
        _c_head_cfg = cfg_override(head_cfg, C1=C_C2, C2=self.output_channels // 3 * 2, Ch=max(C_C2, 2), kernel_size=1, act='tanh')
...
def build_encoding(args, logger, size, channels):
    ...
    cfg = {
        ...
        'glob_chans': args.base_glob_chans
    }
...
def set_args(parser):
    ...
    for prefix in ['base']:
        ...
        group.add_argument(f'--{prefix}-glob-chans', type=int, default=1, help=f'{prefix.title()} global feature channels multiplier.')
