...
- multiply one-channel feature from Luma path to the features in Chroma path
# output
        ...
        for i, (l_block, c_block) in enumerate(zip(self.l_blocks, self.c_blocks)):
            ...
            for _, l_layer in enumerate(l_block):
                if isinstance(l_layer, NeRVUpsampler):
                    ...
                    l_x, mask, cross_lc = l_layer(idx[:, [0]], l_x, px_mask_3d)
                    if mask is not None:
