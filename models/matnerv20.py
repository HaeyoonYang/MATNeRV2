...
# output:
    low-freq path: [N, T, H, W, 3]
    high-freq path: [N, T, H, W, 1]         # intermediate mask feature (w/ loss applied to it)
"""
from .utils import *
from .layers import *
...
class NeRVDecoder(nn.Module):
    ...
    def __init__(self, logger, input_size, input_channels, output_size, output_channels,
        ...
        # Conv blocks
        _block_cfg = cfg_override(block_cfg, type=low_cfg['block_type'], C1=C1, C2=C2, Ch=int(C2 * self.exps[i]), kernel_size=self.kernels[i], depths=self.depths[i], mat_dim=(output_size[0], block_cfg['mat_chans']))
        ...
        # Conv blocks
        _block_cfg = cfg_override(block_cfg, type=high_cfg['block_type'], C1=C1, C2=1, Ch=int(self.exps[i]), kernel_size=self.kernels[i], depths=self.depths[i], mat_dim=(output_size[0], block_cfg['mat_chans']))
        self.high_branch.append(get_block(**_block_cfg))
        self.high_act = get_activation(high_cfg['act'])()
...
class NeRV(nn.Module):
    ...
    def forward(self, input):
        ...
        output, high_output = self.decoder(output, idx, idx_max, patch_mode=patch_mode)
        ...
        high_output = high_output.permute(0, 4, 1, 2, 3).contiguous(memory_format=torch.channels_last_3d)

        return output, high_output
...
def set_args(parser):
    ...
    group.add_argument('--high-act', type=str, default='tanh', help='type of activation for high-freq branch output (tanh / sigmoid / none)')