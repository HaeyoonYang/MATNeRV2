...
--> make Head2 deeper (add conv block before Head2)
...
class NeRVDecoder(nn.Module):
    ...
    def __init__(self, logger, input_size, input_channels, output_size, output_channels,
                 ...
                 stem_paddings, stem_cfg, head_cfg, c_block_cfg):
        super().__init__()
        ...
        for _ in range(c_block_cfg['depths']):
            _c_block_cfg = cfg_override(c_block_cfg, C1=C2, C2=C2, kernel_size=3)
            self.c_blocks.append(get_block(**_c_block_cfg))
    ...
    def forward(self, x: torch.Tensor, idx: torch.IntTensor, idx_max: tuple[int, int, int], patch_mode: bool=True):
        ...
        # Chroma block
        c_x = x.detach()
        for _, layer in enumerate(self.c_blocks):
            c_x = layer(c_x)
...
def build_decoder(args, logger, input_size, input_channels, output_size, output_channels):
    ...
    for prefix in ['block', 'enc', 'upsample', 'stem', 'head', 'c_block']:
        cfg[f'{prefix}_cfg'] = {}
