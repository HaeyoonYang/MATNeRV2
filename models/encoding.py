...
Index to coordinate
"""
class NormalizedCoordinate(nn.Module):          # normalize to [-1, 1]
...
class NormalizedCoordinate2(nn.Module):         # normalize to [0, 1]
    def __init__(self, align_corners=False):
        super().__init__()
        self.align_corners = align_corners
...
def interpolateAS(grid: torch.Tensor, grid_intv: torch.Tensor, coor_t: torch.Tensor):
    idx_n = torch.clamp(torch.searchsorted(grid_intv, coor_t, right=True), 1, len(grid_intv)-1)
    # idx_n = torch.sum(coor_t >= grid_intv, dim=-1).clamp(1, len(grid_intv)-1)
    idx_p = idx_n - 1
    denom = (grid_intv[idx_n] - grid_intv[idx_p]) + 1e-9
    lerp_w0 = ((grid_intv[idx_n] - coor_t) / denom)[..., None, None, None]
    lerp_w1 = ((coor_t - grid_intv[idx_p]) / denom)[..., None, None, None]
    enc_i = grid[idx_p] * lerp_w0 + grid[idx_n] * lerp_w1
    return enc_i
...
class TemporalLocalGridEncodingwAS(nn.Module):
    def __init__(self, K, C, grid_size, grid_level, grid_level_scale, init_scale, intv_init, align_corners=True):
        super().__init__()
        self.K = K
        self.C = C
        self.grid_sizes = []
        ...
        self.grid_level_scale = grid_level_scale
        self.init_scale = init_scale
        self.intv_init = intv_init
        self.align_corners = align_corners

        # Weights (saved in 2D to prevent converted by .to(channels_last))
        self.grids = nn.ModuleList()
        self.grid_intv = nn.ParameterList()
        for i in range(self.grid_level):
            T_grid_i, C_grid_i = tuple((int(grid_size[d] / (self.grid_level_scale[d] ** i)) for d in range(2)))
            self.grid_sizes.append((T_grid_i, 1, 1, self.K[0], self.K[1], self.K[2], C_grid_i))
            self.grids.append(FeatureGrid((T_grid_i, self.K[0] * self.K[1] * self.K[2] * C_grid_i), init_scale=self.init_scale))
            self.grid_intv.append(GridInterval(T_grid_i-1, init_type=self.intv_init))
    ...
    def forward(self, coor_t: torch.FloatTensor, coor_h: Optional[torch.FloatTensor]=None, coor_w: Optional[torch.FloatTensor]=None):
        ...
        for i in range(self.grid_level):
            ...
            # Interpolate in temporal dimenions
            weight_i = self.grids[i]().view(T_grid_i, H_grid_i, W_grid_i, K0 * K1 * K2 * C_grid_i)
            intv_i = self.grid_intv[i]()
            enc_i = interpolateAS(weight_i, intv_i, coor_t)
            enc.append(enc_i.contiguous().view(N, T * H * W * K0 * K1 * K2, C_grid_i))

        return self.linear(torch.concat(enc, dim=-1)).view(N, T, H, W, self.K[0], self.K[1], self.K[2], self.C)
...
class PositionalEncoder(nn.Module):
    ...
    def __init__(self, scale, channels, cfg=None):
        ...
        elif self.coor_type == 'normalized2':
            self.coor = NormalizedCoordinate2(align_corners=cfg['align_corners'])
        ...
        elif self.enc_type == 'temp_local_grid_intv':
            self.enc = TemporalLocalGridEncodingwAS(K=self.scale, C=self.channels, grid_size=cfg['grid_size'], grid_level=cfg['grid_level'], grid_level_scale=cfg['grid_level_scale'], init_scale=cfg['grid_init_scale'], intv_init=cfg['grid_intv_init'], align_corners=cfg['align_corners'])
            self.f_type = 'temp_local'
