...
class NeRVEncoding(nn.Module):
    """
    NeRV Encoding, i.e., the feature grid layer.
    """
...
class NeRVDecoder(nn.Module):
    ...
    def __init__(self, logger, input_size, input_channels, output_size, output_channels,
                 channels, channels_reduce, channels_reduce_base, channels_min,
                 depths, exps, kernels, scales, paddings, stem_kernels, stem_paddings,
                 block_cfg, upsample_cfg, enc_cfg, stem_cfg, head_cfg):
        super().__init__()
        ...
        for i, scale in enumerate(self.scales):
            ...
            # Upsampler
            _upsample_cfg = cfg_override(upsample_cfg, in_channels=C1, out_channels=C2, scale=scale)
            ...
            # Conv blocks
            _block_cfg = cfg_override(block_cfg, type=block_cfg['types'][i], 
                                      C1=C1, C2=C2, Ch=int(C2 * self.exps[i]),
                                      kernel_size=self.kernels[i], depths=self.depths[i],
                                      mat_dim=(output_size[0], block_cfg['mat_chans']))
        ...
        _head_cfg = cfg_override(head_cfg, C1=C2, C2=self.output_channels,
                                 Ch=max(C2, self.output_channels), kernel_size=1)
        ...
        for i, block in enumerate(self.blocks):
            ...
            for _, layer in enumerate(block):
                if isinstance(layer, HiNeRVUpsampler):
                    ...
                    x, mask = layer(idx[:, 0:1], x, px_mask_3d)
                    if mask is not None:
...
def build_model(args, logger, input):
    ...
    args.base_channels = sum([int(args.base_grid_size[3] // args.base_grid_level_scale[3] ** i) for i in range(args.base_grid_level)]) * 3
...
def set_args(parser):
    ...
    # Model complexity
    group.add_argument(f'--channels', type=int, default=256, help='channels of NeRV')
    group.add_argument(f'--channels-reduce', type=float, default=2.0, help='channels reduction factor of NeRV')
    group.add_argument(f'--channels-reduce-base', type=int, default=1, help='the base number which force to be a factor of channels of NeRV')
    group.add_argument(f'--channels-min', type=int, default=1, help='min channels of NeRV')
    group.add_argument(f'--depths', type=int, nargs='+', default=[3, 3, 3, 1], help='depths of NeRV')
    group.add_argument(f'--exps', type=float, nargs='+', default=[4., 4., 4., 1.], help='expansion ratio of NeRV')
    ...
    group.add_argument(f'--block-types', type=str, nargs='+', help=f'type of blocks for each layer')
    group.add_argument(f'--block-mat-chans', type=int, default=2, help=f'type of blocks for each layer')
    ...
    # Upsample layers
    group.add_argument('--upsample-type', type=str, default='trilinear', help='Upsampling method of NeRV')
    group.add_argument('--upsample-config', type=str, default='matmul-th-w', help='Upsampling method config of NeRV')
    ...
    # Wrapper
    group.add_argument('--eval-patch-size', type=int, nargs='+', default=None, help='patch size during evalution for NeRV')
...
class GridTrilinear3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_p = torch.roll(x, shifts=1, dims=0)
        x_p[0] = x_p[1]
        x_n = torch.roll(x, shifts=-1, dims=0)
        x_n[-1] = x_n[-2]
        out = torch.cat([x_p, x, x_n], dim=-1)              # [T, H, W, 3C]
        return out
