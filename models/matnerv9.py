"""
grid --> Blocks --> 3rd block (L) --> 4th block --> Head1 --> Y channel
                        -//-> 3rd block (C) --> 4th block --> Head2 --> CoCg channels

Divide the last two block into two branches (Luma / Chroma)
The input to chroma path is detached, so that the main network focuses on Y channel only.
...
class NeRVDecoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, idx: torch.IntTensor, idx_max: tuple[int, int, int], patch_mode: bool=True):
        ...
        for i, block in enumerate(self.blocks):
            # detach chroma path before the last two block
            if i == self.backbone_depths:
                c_x = x.detach()
