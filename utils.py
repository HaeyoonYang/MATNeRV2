...
def get_output_path(args):
    ...
    #     output_dir = '/'.join(args.resume.split('/')[:-2])
    ...
    output_dir = os.path.join(args.output, args.model, exp_name) if len(exp_name) else args.output
    output_dir, = accelerate.utils.broadcast_object_list([output_dir], 0)
    return output_dir
...
class CheckpointManager:
    ...
    def save_latest(self, epoch):
        if self.accelerator.is_main_process:
            new_path = os.path.join(self.output_dir, 'checkpoint_latest')
            self.accelerator.save_state(output_dir=new_path)
            self._save_meta(new_path, {'epoch': epoch})
