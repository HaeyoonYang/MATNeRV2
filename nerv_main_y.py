...
from nerv_tasks_y import VideoRegressionTask, set_task_args
...
group.add_argument('--model', default='NeRV', type=str, help='Model to build, e.g. NeRV.')
...
def main():
    ...
    if not args.eval_only:
        ...
        while epoch < args.epochs:
            ...
            checkpoint_manager.save_latest(epoch)
        ...
        while epoch < args.epochs + args.prune_epochs + args.quant_epochs:
            ...
            if do_eval(args, quant_epoch, args.quant_epochs):
                _, metrics = eval_step(args, logger, '', epoch, model, eval_loader, eval_task, accelerator, True)
                checkpoint_manager.save(epoch, metrics)
        ...
        # Evaluation
        _, metrics = eval_step(args, logger, None, epoch, model, eval_loader, eval_task, accelerator, args.log_eval)
